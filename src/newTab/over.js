...
async function initDailyJournal() {
  ...
  const { year, monthDay, history, yearObj, dayObj, journalList } = await getDailyJournalState(currentDate)
...
applyPollConfig(
  pollBtn,
  document?.getElementById('votePoll'),
  { text: CONFIG.LINK_TITLE, url: CONFIG.LINK_URL },
  (url) => chrome.tabs.create({ url })
)
